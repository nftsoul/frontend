                                                ...
                                                <v-list-item class="px-0">
                                                    ...
                                                    <v-list-item-action>
                                                        <v-tooltip top v-if="collected.includes(item.name)">
                                                            ...
                                                            <span class="caption" style="z-index: 5000">Already belongs
                                                                to a collection</span>
                                                        </v-tooltip>
                                                        <div v-else @click="selectNft(item)" class="link">
                                                            <v-checkbox v-if="collectionIds.includes(item.image)"
                                                                disabled v-model="yes" color="green" dark
                                                                style="border-radius: 50% !important"></v-checkbox>
                                                            <v-checkbox v-else disabled v-model="no" color="green" dark
                                                                style="border-radius: 50% !important"></v-checkbox>
            ...
            if (this.wallet == "Phantom") {
                ...
                if (this.cluster == "devnet") {
                    ...
                    for (var x = 0; x < this.meta.length; x++) {
                        try {
                            promises.push(await axios.get(this.meta[x].data.uri).then((response) => {
                                if (response.data) {
