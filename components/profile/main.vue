                ...
                <v-col cols="6" align="center" class="pb-8">
                    <client-only>
                        <div v-if="profile.image_link" class="mb-3">
                            <v-img :lazy-src="profile.image_link" class="rounded-circle" :src="profile.image_link"
                                alt="Avatar" max-width="160" max-height="160">
                    ...
                    <div v-if="profile != null" class="my-3">
                        ...
                        <v-row justify="center py-3">
                            <p class="mr-5 mt-1 text-gradient link" @click="tutorProfile()" v-if="profile.username">
                                @{{ profile.username }}</p>
                            <v-card max-width="200" dark color="black" class="pa-2" height="40" @click="copy()">
                                <p v-if="profile" class="mb-n7">
                                    {{ profile.wallet_address.slice(0, 8) + '.............' +
                                            profile.wallet_address.slice(-3, -1)
                                    }}
                                </p>
                                <client-only v-else>
                    ...
                    <ShareNetwork network="twitter" :url="getProfileLink()"
                        title="NFTsoul..Exhibit and earn from your NFT Collections"
                ...
                <v-card-text>
                    <v-col align="center">
                        <v-form v-model="valid" ref="form">
                            <v-text-field v-model="name" color="white"
                                :rules="[validRules.required, validRules.lengthMin3]" dense outlined
                                label="Enter your full name"></v-text-field>
                            <v-text-field v-model="username" color="white"
                                :rules="[validRules.required, validRules.lengthMin3]" dense outlined
                                label="Pick a username"></v-text-field>
                        </v-form>
                        <v-btn dark small outlined class="my-n5" @click="updateProfileDetail" :loading="updating">Update
                        </v-btn>
...
export default {
    ...
    methods: {
        ...
        async getAccountInfo() {
            ...
            this.balance = parseFloat(blc * 1e-9).toFixed(5);
        },
        updateProfileDetail() {
