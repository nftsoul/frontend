          ...
          <v-col cols="12" lg="8" md="6">
            <v-row v-if="collections.length == 0" justify="center">
              ...
              <v-col v-else align="center">
                ...
                <ReusableGradientButton @click="createCol()" ButtonText="Create Now" />
              </v-col>
            </v-row>
            <v-row v-else>
                    ...
                    <div class="outer-card">
                      <div class="inner-card">
                        <v-img :src="item.image" :lazy-src="item.image" class="mx-auto" width="220" height="220">
                          <v-expand-transition v-if="walletAddress == profile.wallet_address">
                            <div v-if="hover" class="
                              ...
                              <v-btn small color="background" @click="deleteItem(item)">
                        ...
                        ">
                          <v-card class="rounded-pill mt-n6" max-width="150" style="">
                            <v-list dense class="py-1">
                              <v-list-item dense class="pa-0">
                                ...
                                <v-list-item-content>
                                  <v-list-item-title class="ml-n2" v-if="item.created_by.name">{{
                          ...
                          <v-card-subtitle class="text-left mx-n3">{{ item.gallery_name.slice(0, 20)
                          }}<span v-if="item.gallery_name.length > 20">..</span></v-card-subtitle>

                          <v-row>
                            <div class="prem-sup-card rounded-lg px-2" v-for="(nft, i) in item.nfts.slice(0, 6)"
                              :key="i">
    ...
    <v-dialog max-width="400" v-model="deleteDialog">
      <v-card color="background">
        ...
        <v-card-actions>
          ...
          <v-spacer></v-spacer>
          <v-btn small outlined @click="deleteConfirm()" :loading="deleting">Confirm</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
